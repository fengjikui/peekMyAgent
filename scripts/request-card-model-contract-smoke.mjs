...
const labels = {
  ...
  contextCompactionUserComposition: ({ messages, opaque }) => `${messages} user messages and ${opaque} encrypted items`,
  contextCompactionEstimate: ({ tokens }) => `local estimate ${tokens} tokens`,
  contextCompactionHistoryEffect: "Replacement history becomes live history.",
  contextCompactionOpaqueContent: "Opaque body unavailable.",
  contextCompactionEstimateEvidence: "Estimate is not next API input.",
  contextCompactionNotHttpEvidence: "Not a model HTTP request.",
    ...
    entry: {
      ...
      semantic_event: {
        ...
        data: {
          ...
          retained_message_roles: { user: 11 },
...
assert.deepEqual(buildTimelineSemanticEventView(semanticLifecycleRequest, commonOptions), {
  ...
  facts: "11 user messages and 1 encrypted items · local estimate 6.2k tokens",
  note: "Replacement history becomes live history. Opaque body unavailable. Estimate is not next API input. Not a model HTTP request.",
});
