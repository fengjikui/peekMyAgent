...
| Codex Desktop managed exact capture | From an **external Terminal** in a disposable project run `pma codex desktop`; review the restart warning, consent, send one non-sensitive chat plus one harmless tool-loop request in the native Desktop UI, verify exact System/Tools/Messages/Response Raw, then quit Desktop and confirm normal relaunch has no capture override. Repeat with `pma codex desktop --select --capture exact` (or `--resume <thread-id> --capture exact`), open the selected conversation after restart, and confirm it cold-resumes into capture while an unrelated thread retains its original provider. For scripted consent use `--capture exact --restart`. Never run this while the validating agent itself is hosted by that Desktop. Automated deterministic coverage is limited to `smoke:codex-app-server-relay`, `smoke:codex-app-server-thread-routing`, `smoke:codex-desktop-installation`, and `smoke:codex-desktop-managed-session`. The restart lifecycle command is intentionally excluded from `release:check:*`; even a direct `npm run smoke:run-codex-desktop-exact` skips safely unless the operator explicitly runs `PEEKMYAGENT_ALLOW_CODEX_DESKTOP_RESTART_SMOKE=1 npm run smoke:run-codex-desktop-exact`. | Supported macOS Codex Desktop, usable subscription login, disposable workspace, external Terminal | Exercises real Desktop restart, current embedded app-server flags, subscription authentication and first-party routes; it can interrupt active Desktop tasks | Redacted report naming app/Desktop versions, consent path, exact evidence, tool-loop result, selected-thread isolation, cleanup and normal-reopen result |
| Codex Desktop rollout observation | From a disposable project run `pma codex desktop --capture rollout`, create one new Desktop chat, send a non-sensitive message, and confirm the waiting Source binds in place; also check `pma codex desktop -c` and `desktop --resume <thread-id>`; deterministic coverage: `npm run smoke:run-codex-desktop`, `npm run smoke:codex-rollout-capture`, and `npm run smoke:codex-viewer-integration` | Installed Codex Desktop and one disposable workspace | Real rollout events and catalog layout vary by Codex version; this evidence is semantic rather than an exact wire request | Redacted pending-to-bound report naming Codex version, stable Source ID, selected thread, and tag/translation result |
| Codex exact Responses capture | `pma codex exec -c 'model_reasoning_effort="xhigh"' "<non-sensitive tool-loop prompt>"`; deterministic coverage: `npm run smoke:codex-exact-proxy`, `npm run smoke:codex-exact-viewer-integration`, and `npm run smoke:run-codex-capture` | Installed Codex CLI and usable ChatGPT subscription authentication | Exercises the private first-party Codex route and real authentication; route/client behavior may change independently of peekMyAgent | Redacted multiround/tool-loop report naming Codex version and validated route shape |
| OpenCode CLI exact integration | `npm run experiment:opencode-real-integration`; deterministic coverage: `npm run smoke:opencode-config`, `npm run smoke:opencode-classification`, `npm run smoke:run-opencode`, `npm run smoke:translation-opencode-cli`, and `npm run experiment:opencode-compaction-local` | Installed/configured OpenCode for the real integration command; the compaction probe uses an isolated local mock only | Real provider output and configured model availability vary; the script creates one temporary OpenCode session and paid model requests. The compaction probe separately exercises real OpenCode server/summarize behavior without credentials | JSON report with OpenCode version, model id, request/source counts, ordinary delta invariant, observed tools, command projection, subagent graph, compaction lifecycle and cleanup result; no prompts, replies or credentials |
| CodeBuddy Code exact Chat integration | `npm run experiment:codebuddy-local-mock`; deterministic PMA coverage: `npm run smoke:codebuddy-config` and `npm run smoke:run-codebuddy`. For a paid real-provider check, configure one non-sensitive test model in user/project `models.json` or export `CODEBUDDY_API_KEY`, then run one non-sensitive `pma codebuddy --print` prompt | Installed CodeBuddy 2.131.0 or the release-candidate version; configured OpenAI-compatible OpenCode endpoint for the paid scenario | The local mock consumes no real credential or Token and verifies `models.json` remains byte-identical while exercising Read, Explore subagent and resume. Paid validation may vary by model/provider. Never copy OpenCode auth files or put a key in argv | Version, file/env credential source shape without value, Chat route, model slot mapping, tool-loop roles, privacy-safe subagent purpose, resumed history, purpose/body conflict attribution, request/response counts, full response, cleanup, redacted headers, unchanged config hash, and no prompt/reply/key in the validation report |
