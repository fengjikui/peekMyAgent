...
| CodeBuddy Code exact Chat integration | `npm run experiment:codebuddy-local-mock`; deterministic PMA coverage: `npm run smoke:codebuddy-config` and `npm run smoke:run-codebuddy`. For a paid real-provider check, export `CODEBUDDY_API_KEY` in a disposable shell and run one non-sensitive `pma codebuddy --print` prompt | Installed CodeBuddy 2.130.0 or the release-candidate version; configured OpenAI-compatible OpenCode endpoint for the paid scenario | The local mock consumes no credential or Token; paid validation may vary by model/provider. Never copy OpenCode auth files or put a key in argv | Version, Chat route, model slot mapping, request/response counts, session reuse, purpose attribution, full response, cleanup, redacted headers, and no prompt/reply/key in the validation report |
