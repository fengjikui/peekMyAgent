...
| Codex selected rollout observation | `pma codex --select`, then inspect the selected Source; deterministic coverage: `npm run smoke:codex-rollout-capture` and `npm run smoke:codex-viewer-integration` | Installed Codex/Desktop and one disposable session | Real rollout events and catalog layout vary by Codex version; rollout evidence is semantic and not an exact wire request | Redacted event-chain and compaction report with exact Codex version |
