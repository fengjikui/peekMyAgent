...
- 下一步审计 Client 状态与数据依赖，再选择 Raw Inspector 或 request-detail cache 作为第二个低耦合边界；file/imported sidecar index 与 cursor 分页留在大 Trace 数据路径阶段实施。
...
2. 抽出 API client 和 request-detail cache。
