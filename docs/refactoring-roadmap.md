...
| 发布 | `0.1.0-alpha.4` 已通过 GitHub OIDC/provenance 发布到 npm `next`；PR 候选运行一次三平台矩阵，main/Release 分别只做精确 merge/tag 完整性检查 | 公开预发布闭环已建立；仍需按发布候选完成真实 Windows/Linux 验证，避免重复自动化矩阵拖慢交付 |
...
- 已完成 semver、锁文件、CHANGELOG、预发布 dist-tag、npm 包实体、Trusted Publisher、GitHub Environment 和 OIDC/provenance 工作流；完整三平台矩阵只在 PR 候选执行一次，main 与 Release 保留精确提交/Tag 完整性门禁。
...
CodeBuddy Code 已作为第二个真实 Harness 的候选 adapter 接入：2.131.0 的真实 CLI 假上游证明了 OpenAI Chat、Read 工具循环、Explore 子 Agent、resume 历史、原生 conversation identity 和进程级模型/endpoint 覆写。purpose 已从绝对标签收敛为 Header/body 交叉验证的证据：异步 suggestion 的共享 session 竞态不会再把普通请求移入幕后，子 Agent purpose 只保留 privacy-safe 类别。实现复用共享协议与 Viewer，只在 adapter 中保留 OpenCode 非敏感配置映射、独立认证边界和 continue/resume 生命周期。当前仍需 hosted CI 与 Windows/Linux 实机验证，不能据此把 adapter API 宣布为稳定 SDK。产品层可借鉴的评测闭环与优先级记录在 [Agent 可观测与调试产品借鉴点](agent-observability-product-notes.md)。

## 阶段 6：公开发布成熟度

任务：

- 已建立 semver、锁文件、CHANGELOG、PR 候选三平台门禁、精确 Release Tag 完整性检查、`next`/`latest` 分流和无长期 Token 的 npm OIDC/provenance 工作流；`0.1.0-alpha.4` 已完成可信发布。后续按[发布手册](releasing.md)复用同树平台证据，不能在 main/Release 重复相同矩阵。
