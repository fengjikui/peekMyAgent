...
`pma codex desktop` 以当前 `cwd` 为项目，先建立带稳定 Source ID 的 `codex_rollout_pending` 观察对象，记录当时同工作区已有的 thread id 基线，再打开 dashboard 和 Desktop 工作区。macOS 通过应用 Bundle ID `com.openai.codex` 交给 LaunchServices 打开目录，避免把内置 CLI 误当安装器并阻塞；其他平台通过集中在 Desktop adapter 中的官方 launcher 打开。Viewer 的 Source catalog 轮询会查找同工作区中不在基线内的第一条可读新 thread，并把原 Source 原地绑定为 `codex_rollout_local`；Source ID 不变，因此等待页、URL 和当前选择不会在绑定时跳到另一条记录。

绑定后 Viewer 增量读取该 thread 对应的 rollout JSONL；正文继续留在 `CODEX_HOME`，选择文件只保存观察模式、工作区、基线/绑定 thread id 与 provenance，不复制进 peekMyAgent SQLite。`pma codex desktop -c` 绑定当前工作区最近一条可读 thread，`desktop --resume <thread-id>` 绑定指定 thread，`desktop --select`/`--list` 保留为高级历史观察入口。Desktop 没有安全的会话级 provider 注入入口，因此这里只提供明确标记的 rollout 语义观察，不能伪装成网络层完整请求；模型、推理等级、权限和审批策略仍由 Codex Desktop 管理。rollout reader 是版本化的可选 adapter：若上游停止保留相应事件或改变格式，PMA 应明确报告该观察入口不可用，而默认精确代理不受这一持久化策略变化影响。
