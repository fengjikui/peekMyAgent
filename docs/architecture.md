...
多 Agent 看板默认以“一个子 Agent 一张卡”展示上下文模式、状态和启动/确认/回流路径，避免顶部流程卡和详情卡重复同一信息；交错时间线与关联证据按需展开，分支超过 6 条时才显示运行中、已完成未回流、已回流筛选。筛选后只生成当前状态的分支和事件。Trace 顶层搜索索引派生摘要而不是 Raw body，可按异常、慢请求、工具和子 Agent 定位请求。结果以 Turn 为归属、以命中请求为证据，每次最多追加 24 条，避免搜索本身重新制造超大 DOM。主栏使用容器条件适配真实栏宽，三栏拖拽或折叠不会再把标题挤成竖排。
