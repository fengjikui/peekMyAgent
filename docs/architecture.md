...
`pma codex --select` 只读取 Codex thread catalog 的元数据，让用户选择一条 thread，并在私有选择文件中保存 thread id。Viewer 随后增量读取该 thread 对应的 rollout JSONL；正文继续留在 `CODEX_HOME`，不复制进 peekMyAgent SQLite。该来源标记为 `codex_rollout_local`、只读且不可由 Viewer 删除，它提供 Codex 语义事件而不是网络层完整请求。
