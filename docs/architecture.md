...
外观偏好由 `ThemeController` 独立管理。控制器只负责从 `localStorage` 水合 `system/light/dark/studio`、写入 Store 的 appearance domain、设置根节点 `data-theme` 和刷新双语选择器；它不拥有 feature HTML，也不改变 Trace 数据。CSS 以 canvas、surface、ink、line、accent、status、code、用户消息气泡、子 Agent branch surface/identity 和行内代码等语义 token 定义主题，功能样式不得按主题复制分支。中栏时间线与右栏证据区共享同一 panel surface，栏位、粘性工具区和导航仅用线条与间距区分；左栏保留低一级 surface，用户气泡只使用低饱和主题色层，不再制造独立的高反差色块。暗色主题使用低亮度石墨表面、可辨识细线与独立气泡/代码前景色，不复用只适合亮色背景的硬编码颜色。`system` 通过媒体查询复用亮色/暗色契约，用户显式选择则覆盖系统偏好。
