...
### Desktop-first 单会话本地观察

`pma codex` 以当前 `cwd` 为项目，先建立带稳定 Source ID 的 `codex_rollout_pending` 观察对象，记录当时同工作区已有的 thread id 基线，再打开 dashboard 和 Desktop 工作区。macOS 通过应用 Bundle ID `com.openai.codex` 交给 LaunchServices 打开目录，避免把内置 CLI 误当安装器并阻塞；其他平台通过集中在 Desktop adapter 中的官方 launcher 打开。Viewer 的 Source catalog 轮询会查找同工作区中不在基线内的第一条可读新 thread，并把原 Source 原地绑定为 `codex_rollout_local`；Source ID 不变，因此等待页、URL 和当前选择不会在绑定时跳到另一条记录。

绑定后 Viewer 增量读取该 thread 对应的 rollout JSONL；正文继续留在 `CODEX_HOME`，选择文件只保存观察模式、工作区、基线/绑定 thread id 与 provenance，不复制进 peekMyAgent SQLite。`pma codex -c` 直接绑定当前工作区最近一条可读 thread，`--resume <thread-id>` 绑定指定 thread，`--select`/`--list` 保留为高级历史观察入口。Desktop 只保证打开工作区，当前公开命令不承诺让已经运行的 Desktop 自动切换到某条既有 thread。

Desktop 默认 `--capture auto` 会明确回退到 rollout 语义观察，因为现有 Desktop 进程没有安全的会话级 provider 注入入口。显式 `--capture proxy`/`exact` 会拒绝并指向 `pma codex capture -- ...`，不能静默把 rollout 冒充网络层完整请求。rollout 来源只读且不可由 Viewer 删除；模型、推理等级、权限和审批策略仍由 Codex Desktop 管理。
...
工具事件与子 Agent 事件筛选会保留所属 Turn 的“机制流程”，避免过滤后只剩离散证据而失去链路语义；异常和慢请求筛选继续保持紧凑。筛选计数统一表示匹配的请求/事件数量，顶部统计则保留子 Agent 实例数、工具调用数与工具结果数，两种口径不混用。
