...
| `src/viewer/request-card-model.js` | 请求身份、上行类别/摘要、跨请求工具事件配对和 Assistant 正文/思考折叠的纯 View Model |
