...
这篇文档记录中文快速上手的演示设计、素材来源和重生成方式。中英文根 README 当前共同引用快速上手的 1024×576 静态审阅封面；超过一分钟的循环 GIF 不再承担首屏讲解。面向读者的主入口改为可暂停、拖动、逐镜头回看、切换字幕和全屏的 HTML 播放器，社交媒体版本则继续导出 MP4。三种交付物都从同一时间线和双尺寸复核帧生成。

## 首屏演示只回答一个问题

> PMA 如何让我从一条用户请求，一路追踪到工具调用、工具结果、最终回答和原始协议？

![从用户请求追踪到原始协议](../assets/demo/source/quickstart/recording/review-1024/03-overview.jpg)
...
历史 README 主 GIF 从这些已复核状态中选出 17 个镜头，输出为 1024×576、65 帧、74 秒、约 4.82 MiB。普通动作停留 3～5.5 秒，协议顺序停留 7.5 秒，两级导航分别停留 4.5 秒和 6.5 秒。上方 UI 与标注使用 0.6 秒渐变；字幕安全区不参与整帧混合，而是在转场中干净切换，避免相邻两句形成双影。镜头计划和输出文件分别为：

- `assets/demo/source/quickstart/readme-gif.zh-CN.json`
- `assets/demo/quickstart-tool-loop.gif`

六帧旧版与 74 秒版本当时已经通过抽帧联系表并排复核。新版移除了大块黑色说明条、弯曲箭头和旧紧凑布局；这段记录用于保存 GIF 迁移证据，不再表示它是当前 README 首屏素材。

## 两级导航场景

历史主 GIF 之外还有一条独立长轨迹，专门回答：
...
- 历史主 GIF 也是基于真实 Viewer 状态的慢速逐帧演示，不伪造产品中不存在的 UI；当前首屏改为静态封面与可控 HTML。
...
若维护历史 README GIF，仍要增加一道成片门禁：`readme-gif.zh-CN.json` 实际选中的每一张画面，都必须分别留下 1920×1080、1024×576 和正式 GIF 抽帧三项复核结果。联系表只用于发现全局节奏和明显异常，不能代替逐张打开；`tmp/` 中的候选 HTML、JPEG 或 GIF 也不能代替正式文件。只有正式文件重建后，再从 GIF 的静态停留段和转场中点抽帧复看，才可以记录“历史 README GIF 已通过”。当前根 README 的静态封面则以对应 1024×576 审阅帧为发布文件。
...
2026-07-31 对 v0.1 的 `01`～`07` 完成了 2048×1056 原图与 1024 像素宽 README 预览复核。这条记录只证明当时公开的 GIF，不自动证明新版故事板。
...
- `assets/demo/quickstart-tool-loop.gif`：历史 README 74 秒慢速 GIF，由双尺寸复核状态重建；当前根 README 不再引用。
...
这条命令只重建历史 v0.1 GIF。v0.2 使用网页故事板：
