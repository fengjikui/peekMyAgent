...
- SQLite 和 live backend 只 hydrate 当前页；file/import backend 使用私有 JSON array sidecar 按 byte range hydrate 当前页。首次遇到 legacy 文件仍需做一次线性结构扫描，但不再完整 `JSON.parse` 或常驻整条 Trace。详见 [JSON Array File Index 契约](json-array-file-index-contract.md)。
