...
Client 以 request/Turn/branch/spawn/return 的稳定 id 合并实体。页面级 delta 字段不进入长期应用状态。
...
- Client 在后台逐页合并后仍按现有 timeline window 重绘；normalized entity/page store 是后续阶段，不应再退回整条 compact 下载。
