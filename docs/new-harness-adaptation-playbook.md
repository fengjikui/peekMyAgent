...
OpenCode 验证了上述手册，也补充了几类只有进入真实协议后才暴露的问题：

| 现象 | 根因 | 处理规则 |
| --- | --- | --- |
| 第一轮尚未知道 session id | 新会话 id 由 OpenCode 创建，wrapper 启动前不存在 | 先用 watch 身份接收请求，再只在 effective Agent 已确认为 OpenCode 时学习 `x-session-id`；不能全局解释同名 header |
| 标题生成混入主时间线 | title request 与主请求都走同一 provider/path | 使用 tools 为空、固定 system/user 结构和 OpenCode provenance 的组合指纹；相同正文在 Claude/Codex 来源中必须是反例 |
| 自定义 slash command 看起来只是 user message | command 展开后 wire role 不携带 command name | wrapper 从真实 `--command` 参数注入仅进程内证据 header，Capture 保存后在上游转发前剥离；History/Message 保留原文，Harness 只做额外投影 |
| 普通第二轮重复显示旧工具结果 | OpenCode 在累计 history 前改变了 leading system 中的 Skill 路径顺序 | conversation delta 对 leading system/developer 只按 role 对齐；system fingerprint 仍单独报告 changed，conversation message 继续严格比较 |
| 标题指纹误伤其他 Harness | 只看 prompt 文本，没有看 capture provenance | Harness 特有模板必须允许“未知来源”兼容旧 fixture，但已知其他 Agent 必须显式拒绝 |
| 压缩测试显示 command rejected | `run --command compact` 调用 generic command，不是 TUI `/compact` | 从官方 server summarize 生命周期验证：独立模型请求、compaction boundary、summary assistant 和下一轮复用；测试入口语义必须与真实功能一致 |
| 子 Agent 看似可按请求相邻顺序关联 | child 请求与主请求会交错，相邻只是一种偶然 | 使用 OpenCode session identity、task call/result 和共享 graph；没有稳定 identity 时不得建立高置信分支 |
| fake smoke 全绿但 Viewer 仍会错 | 协议测试只证明转发，不证明信息组织 | 每个新 Harness 至少做一次真实多轮并打开三栏 Viewer，检查普通轮次没有历史污染、工具闭环和 Harness 投影位置正确 |

由此增加四条适配纪律：

1. **先算 effective Agent，再解释 Agent 专属 header。** fallback profile、route 和本地 evidence header 的优先级必须由 attribution contract 锁定。
2. **固定上下文变化与对话历史增量分开计算。** System/Tools 变更不能把累计 History 重置成“全新消息”。
