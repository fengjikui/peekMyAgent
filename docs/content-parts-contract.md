...
- 统一 Anthropic `tool_use` / `server_tool_use` 与 OpenAI-compatible `tool_calls`；
- 统一 role=`tool` 与 Anthropic client/server tool result blocks；
...
- 一个 Anthropic user message 可以同时包含 client/server tool result 与后续 Harness text；工具结果提取只返回已知 result block 部分。
