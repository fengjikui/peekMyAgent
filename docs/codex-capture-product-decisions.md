...
技术事实是：平台原生 launcher 可以把一个目录交给 Desktop 打开，但现有 Desktop 进程持有自己的 app-server 连接，不会自然继承 CLI 子进程的一次性 provider，也不能在运行中接收新的进程环境。macOS 使用 `com.openai.codex` Bundle ID，而不是可能触发大体积安装下载的内置 `codex app`。因此当前已实现入口不通过全局配置污染、系统级 TLS MITM 或跨平台 UI 自动化强行接管既有 Desktop；需要原生 Desktop 交互时仍使用显式 rollout 观察。

2026-07-19 的隔离实验确认了一个后续可产品化的受管路径：当前 Desktop 可在启动时通过 WebSocket override 连接版本匹配的外部 App Server；透明 relay 能完整观察 Desktop 与 App Server 的双向 JSON-RPC，且 Desktop 不再启动自己的私有 App Server。未来可将该 relay 与现有 Capture Proxy 组合，形成“原生 Desktop UI + Harness 控制面 + 模型 wire 面”的精确捕获。它需要由 PMA 受管启动或重启 Desktop，不能宣称为原地附着已运行进程。详见 [Codex Desktop App Server Bridge 实验](experiments/codex-desktop-app-server-bridge-2026-07-19.md)。
