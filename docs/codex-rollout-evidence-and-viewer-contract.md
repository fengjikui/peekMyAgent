...
重建 body 保留 OpenAI Responses 的规范 `input`，不再同时写入内容相同的派生 `messages`。共享 Trace Domain 在需要角色语义时从 `input` 投影 message；Raw 的完整请求因此不会伪造第二份历史，体积统计也不计算重复副本。完整请求标签页只渲染一次完整对象，System、Developer、Tools、Harness、History、Message 和 Metadata 由各自标签页提供整理视图。
...
标签解析采用平衡扫描。若注入正文为了讲解规则再次出现一对同名标签，内层示例属于外层正文，不会提前结束区块。`multi_agent_mode` 作为编排启动策略单独展示；无标签的 developer 正文只在命中经过真实 rollout 验证的强指纹时识别为 Memory 或多 Agent 编排，普通正文只进入独立 Developer 标签。已识别的 developer 子块会同时投影到 Harness，便于按语义翻译和检索，但不会从 Developer 原文中删除。

slash 命令注入采用同一双层规则：Claude 的显式 `<command-name>` / `<command-message>` 与 Codex `/compact` 的固定接手提示进入 Harness；原始 message 仍按原顺序保留在 History/Message。Codex 压缩接手消息中只有固定指令前缀作为 Harness 翻译材料，后续 replacement summary payload 不复制进 Harness，避免把历史正文误称为框架提示词。未知标签、未知命令和全部原始 role、顺序、正文继续保留在 Raw 中，不做关键词猜测。
...
Codex `spawn_agent` 是启动尝试；只有对应 `function_call_output` 返回 `agent_id`（新版）或 task name（旧版）才建立子 Agent 分支，参数校验失败等回执只保留为失败尝试证据，不得制造一个永久“运行中”的假分支。精确代理从子请求 `client_metadata.thread_id` 恢复实例 ID，并与启动回执的 `agent_id` 配对，因此子线程自己的模型请求、工具调用和工具结果会成为该分支的完整步骤。新版 `wait_agent` 的终态 `function_call_output` 或同义 `subagent_notification` 标记结果回流；旧版带 `FINAL_ANSWER` 的 `agent_message` 仍受支持。看板同时显示稳定子 Agent 编号、spawn/launch/return 请求号和关联信号。任务正文若在 rollout 中仅保存为加密载荷，看板只说明其可见性而不复制密文；明文任务则显示受限摘要。紧凑 Timeline 会保留这些 semantic annotations 与 `subagent_result` entry，使后续分页或重新组图时不依赖已裁掉的 Raw body。原始 call id、author、recipient 与 rollout event 仍可在详情中查看。

当同一上行增量同时包含 `wait_agent` 的 `function_call_output` 与 Harness 注入的 `<subagent_notification>` 时，请求卡以真实 `tool_result` 为主类型，Harness 通知继续保留在 Harness 详情中；两者是同一完成事实的两类证据，不重复制造两个事件。
