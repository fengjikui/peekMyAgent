...
- `src/viewer/agent-graph-renderer.js`：根据显式 View DTO 生成常驻的 child Agent tabs、一个选中分支的完整时间线容器和关联证据 HTML。

## 边界

两个模块都不得读取全局 `state`、访问 DOM 或发送网络请求。`client.js` 继续负责：

- 从应用状态读取每个 Turn 的选中分支；
...
- 看板不再增加一层总览 disclosure；第一行始终显示 child Agent tabs，一位 child Agent 一个 tab。真实 spawn nickname/分支 label 是主标签，`子1` 等通用编号只能作为辅助信息。
...
- Agent tab 切换只原位替换当前 Turn 的分支区域，不重建整条 Timeline；仍只生成一个选中分支，避免为不可见 child 分支付出 DOM 成本。

`scripts/agent-graph-view-contract-smoke.mjs` 直接锁定真实 nickname 优先级、稳定颜色/glyph、常驻 tab、单一完整分支 timeline、次级关联证据、HTML 转义和无副作用边界。`scripts/timeline-window-smoke.mjs` 锁定 child request 去重与应用装配。真实浏览器回归还需覆盖 tab 首击切换、分支 request Raw、窄栏横向 tabs 和多主题。
