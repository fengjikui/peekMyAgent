...
| OpenCode、CodeBuddy、OpenClaw、Trae 或新 Agent | 已通过标准 OpenAI/Anthropic base URL 的自研 Harness 先从 `bin/peekmyagent.mjs` 的 `runObservedHarness`、`scripts/observe-wrapper-smoke.mjs` 和共享 protocol/response contract 开始；它只提供协议级 exact capture，不推断 Harness 私有语义。OpenCode 专用路径从 `runOpenCodeAgent`、`src/adapters/opencode-config.mjs` 和 `scripts/opencode-*-smoke.mjs` 开始。CodeBuddy 从 `runCodeBuddyAgent`、`src/adapters/codebuddy-config.mjs`、子进程 `codebuddy-model-config-hook.cjs`、`src/core/redaction.mjs` 的 adapter-scoped safe Header semantics、`src/trace/request-profile.mjs` 的 Header/body 交叉归属、`scripts/codebuddy-*-smoke.mjs` 和 `scripts/run-codebuddy-wrapper-smoke.mjs` 开始；真实安装包的 Read/subagent/resume 假上游证据见 `scripts/codebuddy-local-mock-probe.mjs`。通用流程见[新 Harness 适配工作手册](new-harness-adaptation-playbook.md)、[OpenCode 专项计划](opencode-cli-adaptation-plan.md)和[CodeBuddy 专项计划](codebuddy-code-adaptation-plan.md) | 在 SourceRepository、Viewer API、`client.js`、renderer 或翻译策略中增加 `agent/provider` 分支，按展示名定义协议或私有机制，先做 Viewer 再补协议证据，混淆 evidence/source，把 Harness purpose Header 当绝对事实，按 `/` 前缀猜 Harness 注入，或把 server event 冒充 wire request |
...
| 请求协议识别、上下行条目/工具生命周期、provider 与 main/subagent/metadata/background 归因 | `src/shared/request-payload.mjs` 统一递归工具目录、限定名和叶子 Raw 路径；`src/trace/request-profile.mjs`、`src/trace/protocol-exchange.mjs`、`src/trace/request-attribution.mjs` 负责协议与归因；见 [Protocol Exchange 契约](protocol-exchange-contract.md)、[官方 Schema 覆盖](protocol-schema-coverage.md)及对应 contract smoke | 在 Server/Viewer/adapter 中散落 model、path、header 或后台任务判断；按 Agent 名称定义协议；把 `additional_tools` 当空 Developer 消息；把 namespace 当可调用工具计数；让 Renderer 重新解析 provider Raw |
...
| Viewer 翻译材料提取、刷新、Developer/Assistant 文本、Harness 提示词、provider 路由与 occurrence | `src/translation/request-materials.mjs`、`src/translation/materials.mjs`、`src/translation/provider-policy.mjs`、`src/server/viewer-translation-adapter.mjs`、`scripts/translate-materials-zh.mjs`、[材料契约](translation-material-contract.md)、[翻译块协议](translation-block-contract.md)、[Adapter 契约](viewer-translation-adapter-contract.md) | 在 HTTP route 或 `client.js` 重写 System/Developer/Tools/Harness/Response 提取、marker/hash/occurrence；自动发送用户/历史消息；或让已知 Agent 被环境变量静默改投另一个 Agent/provider |
...
| 单请求中的图片/base64 与大型工具结果按字段加载 | `src/contracts/lazy-payload.mjs`、`src/server/lazy-payload-service.mjs`、`request-detail-cache.js`、`client.js` 装配、[字段级懒加载契约](lazy-payload-contract.md) | Renderer 自行猜 base64/工具结果、把 ref 变成任意 JSON path 读取器、未点击即把内容或图片写入 DOM、允许 SVG/远程上传，或把当前实现描述为零 Server 水合 |
...
| 中栏 Timeline、Turn/Request 两级导航、Turn 机制流程、请求卡、多 Agent 看板、上行详情 | `trace-timeline-*`、`turn-rail.js`、`request-rail.js`、`turn-story-*`、`request-card-model.js`、`request-card-renderer.js`、`agent-graph-*`、`upstream-detail-*`、[请求卡 View 契约](request-card-renderer-contract.md)；Request 导航是中栏横向 sticky 上下文条，从首次出现起就使用完整固定刻度轨道并复用 Turn Rail 的悬停涟漪，tooltip 只显示编号和截断的上行问题；滚动选择与 Raw request 分离；multi-Agent 看板位于发起请求之后并默认折叠，展开后 child tabs 只渲染一个完整分支时间线；完整参数、结果和统计进入右侧证据栏 | 在 `client.js` 重写请求分类/摘要/工具配对，在机制流程中按 provider 分支，让 renderer 读取全局 `state`/DOM，把 Raw/统计重新塞入中栏，或用 Timeline 滚动状态覆盖 Raw 详情上下文 |
| Raw、Protocol、Metadata、Messages、工具调用与翻译展示 | `raw-inspector-controller.js`、`raw-view-model.js`、`raw-*`、`protocol-exchange-view-*`、`metadata-view-model.js`、`metadata-renderer.js`、`message-*`、`tool-call-view-model.js`、`tool-call-renderer.js`、`translation-*`、[Protocol Exchange 契约](protocol-exchange-contract.md)、[Raw Inspector Controller 契约](raw-inspector-controller-contract.md)、[翻译视图契约](translation-view-renderer-contract.md)、[Raw 搜索浏览器契约](raw-search-browser-contract.md)；Protocol 只展示协议原顺序、工具阶段和 Raw 跳转；Metadata 原文保留上行捕获字段，整理视图明确区分厂商返回、上行请求字段、捕获事实、PMA 规范化和 PMA 计算，并按原生路径有界展示 OpenAI/Anthropic 的模型、推理、采样、输出、工具与执行参数；Tools schema 以一个工具一个整体展示，Assistant 可按当前下行工具名过滤上行参考；工具调用整理保留原生类型/名称/call id，并只把参数改为代码块或紧凑字段；Raw Response 保留厂商协议原始字段，统一摘要只服务整理视图 | `client.js` 新增长段领域 HTML，或绕过 Controller 直接提交异步 Raw 结果；Renderer 重新解析 provider Raw；Raw 搜索只检查截断预览而不检查完整值；把 Tools schema 误称为 Response 内容；把 PMA 计算伪装成协议字段；把大型 schema、prompt、用户 metadata 或加密内容复制进参数摘要；把统一 `text/thinking/tool_calls` 再拼回 Raw Response；为不同 Harness 复制工具调用页面 |
...
| 外观主题与主题偏好 | `theme-controller.js`、`client-store.js` 的 appearance domain、`styles.css` 根语义 token、`ui-i18n.js` | 在 feature CSS 中添加按主题复制的选择器、写死仅亮色可用的背景，或让主题切换改变 Trace/翻译数据 |
...
- PR 的只读文档影响链由 `scripts/documentation-consistency-audit.mjs`、`scripts/documentation-impact-summary.mjs` 和 `.github/workflows/release-check.yml` 共同拥有：前者映射 base...head，摘要器校验精确 SHA，工作流只写 Job Summary；不得在这条链上增加隐式发布、PR 写权限或未经去重的外部任务创建。
