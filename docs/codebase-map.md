...
| 中栏 Timeline、Turn/Request 两级导航、Turn 机制流程、请求卡、多 Agent 看板、上行详情 | `trace-timeline-*`、`turn-rail.js`、`request-rail.js`、`turn-story-*`、`request-card-model.js`、`request-card-renderer.js`、`agent-graph-*`、`upstream-detail-*`、[请求卡 View 契约](request-card-renderer-contract.md)；Request 导航是中栏横向 sticky 上下文条，滚动选择与 Raw request 分离；child tabs 只渲染一个完整分支时间线；完整参数、结果和统计进入右侧证据栏 | 在 `client.js` 重写请求分类/摘要/工具配对，在机制流程中按 provider 分支，让 renderer 读取全局 `state`/DOM，把 Raw/统计重新塞入中栏，或用 Timeline 滚动状态覆盖 Raw 详情上下文 |
