...
| OpenCode、CodeBuddy、OpenClaw、Trae 或新 Agent | 已通过标准 OpenAI/Anthropic base URL 的自研 Harness 先从 `bin/peekmyagent.mjs` 的 `runObservedHarness`、`scripts/observe-wrapper-smoke.mjs` 和共享 protocol/response contract 开始；它只提供协议级 exact capture，不推断 Harness 私有语义。OpenCode 专用路径从 `runOpenCodeAgent`、`src/adapters/opencode-config.mjs` 和 `scripts/opencode-*-smoke.mjs` 开始。CodeBuddy 从 `runCodeBuddyAgent`、`src/adapters/codebuddy-config.mjs`、子进程 `codebuddy-model-config-hook.cjs`、`scripts/codebuddy-*-smoke.mjs` 和 `scripts/run-codebuddy-wrapper-smoke.mjs` 开始；真实安装包假上游证据见 `scripts/codebuddy-local-mock-probe.mjs`。通用流程见[新 Harness 适配工作手册](new-harness-adaptation-playbook.md)、[OpenCode 专项计划](opencode-cli-adaptation-plan.md)和[CodeBuddy 专项计划](codebuddy-code-adaptation-plan.md) | 在 SourceRepository、Viewer API、`client.js`、renderer 或翻译策略中增加 `agent/provider` 分支，按展示名定义协议或私有机制，先做 Viewer 再补协议证据，混淆 evidence/source，按 `/` 前缀猜 Harness 注入，或把 server event 冒充 wire request |
