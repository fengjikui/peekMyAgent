...
| Viewer HTTP 安全和 API | `src/contracts/viewer-api.mjs`、`http.mjs`、`viewer-router.mjs`、[DTO 契约](viewer-api-dto-contract.md)、[Router 契约](viewer-router-contract.md)、`src/viewer/api-client.js` | 在 `server.mjs` 重新写 URL/method/intent 分支、分别解释 Source/request detail 字段，或让 feature renderer 发 `fetch` |
