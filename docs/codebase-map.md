...
| OpenCode、OpenClaw、Trae 或新 Agent | OpenCode 从 `bin/peekmyagent.mjs` 的 `runOpenCodeAgent`、`src/adapters/opencode-config.mjs`、`scripts/opencode-*-smoke.mjs` 和 `scripts/run-opencode-wrapper-smoke.mjs` 开始；真实集成/压缩机制见 `scripts/opencode-real-integration-smoke.mjs`、`scripts/opencode-compaction-local-probe.mjs`；通用流程见[新 Harness 适配工作手册](new-harness-adaptation-playbook.md)和[OpenCode 专项计划](opencode-cli-adaptation-plan.md) | 在 SourceRepository、Viewer API、`client.js`、renderer 或翻译策略中增加 `agent/provider === "opencode"` 分支，先做 Viewer 再补协议证据，混淆 evidence/source，按 `/` 前缀猜 Harness 注入，或把 server event 冒充 wire request |
