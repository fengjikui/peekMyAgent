...
| 请求协议识别、上下行条目/工具生命周期、provider 与 main/subagent/metadata/background 归因 | `src/trace/request-profile.mjs`、`src/trace/protocol-exchange.mjs`、`src/trace/request-attribution.mjs`、[Protocol Exchange 契约](protocol-exchange-contract.md)及对应 contract smoke；namespace 容器递归展开、限定名和叶子 Raw 路径由协议投影统一负责 | 在 Server/Viewer/adapter 中散落 model、path、header 或后台任务判断；按 Agent 名称定义协议；把 `additional_tools` 当空 Developer 消息；把 namespace 当可调用工具计数；让 Renderer 重新解析 provider Raw |
