...
| 请求协议/provider 与 main/subagent/metadata/background 归因 | `src/trace/request-profile.mjs`、`src/trace/request-attribution.mjs`、对应 contract smoke | 在 Server/Viewer/adapter 中散落 model、path、header 或后台任务判断 |
...
| Viewer 翻译材料提取、刷新、Harness 提示词、provider 路由与 occurrence | `src/translation/request-materials.mjs`、`src/translation/materials.mjs`、`src/translation/provider-policy.mjs`、`src/server/viewer-translation-adapter.mjs`、`scripts/translate-materials-zh.mjs`、[材料契约](translation-material-contract.md)、[翻译块协议](translation-block-contract.md)、[Adapter 契约](viewer-translation-adapter-contract.md) | 在 HTTP route 或 `client.js` 重写 System/Tools/Harness 提取、marker/hash/occurrence，或让已知 Agent 被环境变量静默改投另一个 Agent/provider |
...
| Raw、Metadata、Messages、工具调用与翻译展示 | `raw-inspector-controller.js`、`raw-view-model.js`、`raw-*`、`metadata-view-model.js`、`metadata-renderer.js`、`message-*`、`tool-call-view-model.js`、`tool-call-renderer.js`、`translation-*`、[Raw Inspector Controller 契约](raw-inspector-controller-contract.md)、[翻译视图契约](translation-view-renderer-contract.md)、[Raw 搜索浏览器契约](raw-search-browser-contract.md)；Metadata 原文保留上行捕获字段，整理视图明确区分厂商返回、捕获事实、PMA 规范化和 PMA 计算；Tools schema 以一个工具一个整体展示，Assistant 可按当前下行工具名过滤上行参考；工具调用整理保留原生类型/名称/call id，并只把参数改为代码块或紧凑字段；Raw Response 保留厂商协议原始字段，统一摘要只服务整理视图 | `client.js` 新增长段领域 HTML，或绕过 Controller 直接提交异步 Raw 结果；Raw 搜索只检查截断预览而不检查完整值；把 Tools schema 误称为 Response 内容；把 PMA 计算伪装成协议字段；把统一 `text/thinking/tool_calls` 再拼回 Raw Response；为不同 Harness 复制工具调用页面 |
